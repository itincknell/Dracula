...
from dracula.bgc_policy_data import load_json
from dracula.bgc_policy_model import PARAMETER_COUNT
...
def load_bgc_policy_training_config(path: str | Path) -> BGCPolicyTrainingConfig:
    ...
    if root["format"] != TRAINING_RUN_FORMAT:
        raise BGCPolicyTrainingError("training configuration format is incompatible")
    ...
    if (
        ...
        raise BGCPolicyTrainingError("training configuration values are invalid")
    return BGCPolicyTrainingConfig(
...
def config_from_resolved(output: Path) -> tuple[BGCPolicyTrainingConfig, int | None]:
    ...
    try:
        ...
        raise BGCPolicyTrainingError("resolved training configuration is invalid") from error
    if smoke_epochs is not None and type(smoke_epochs) is not int:
        raise BGCPolicyTrainingError("resolved smoke epoch count is invalid")
