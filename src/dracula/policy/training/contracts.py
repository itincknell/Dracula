...
# Every target row is the 128 root visits produced by information-set UCT.
OUTER_SIMULATION_BUDGET = 128

# Optimization values are deliberately fixed rather than repeated in each TOML
# run file. A run chooses only its seed, snapshot, output directory, and device.
LEARNING_RATE = 3e-4
BETAS = (0.9, 0.999)
EPSILON = 1e-8
WEIGHT_DECAY = 1e-4
...
class PolicyTrainingError(ValueError):
    """A corpus, snapshot, batch, configuration, or artifact is invalid."""


class PolicyTrainingInterrupted(RuntimeError):
...
class PolicyTrainingConfig:
    ...
    def output_path(self) -> Path:
        """Return the absolute run directory used for all generated artifacts."""

        return Path(self.run.output_directory).expanduser().resolve()

    @property
    def snapshot_path(self) -> Path:
        """Return the absolute location of the sealed input corpus."""
