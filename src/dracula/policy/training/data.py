...
Corpus readers and training checkpoints both consume sealed JSON and packed
Boolean fields. This module owns their shared byte-level operations. Dataset
structure and mutable checkpoint state remain with their respective boundaries.
"""
...
def canonical_json(value: object) -> bytes:
    ...
    try:
        ...
        raise PolicyTrainingError("artifact is not canonical JSON") from error
...
def decode_packed(
    ...
    *,
    ...
    if not isinstance(value, str):
        raise PolicyTrainingError(f"{label} must be base64 text")
    try:
        decoded = base64.b64decode(value, validate=True)
    except (ValueError, binascii.Error) as error:
        raise PolicyTrainingError(f"{label} is not canonical base64") from error
    if len(decoded) != byte_count:
        raise PolicyTrainingError(f"{label} has the wrong byte count")
    unused = byte_count * 8 - bit_count
    if unused and decoded[-1] & ((1 << unused) - 1):
        raise PolicyTrainingError(f"{label} has nonzero padding bits")
    return decoded
