...
from dracula.decision.action_mask import (
    representative_action_mask_from_groups,
...
from dracula.game.cards import CARD_COUNT, CARD_INDEX_BY_ID
...
def _encode_coffin(
    ...
    """Write the nine position-by-card occupancy blocks."""
...
def _encode_card_status(
    observation: Tensor,
    information: SearchInformationState,
) -> None:
    """Write the played, in-hand, and unseen card-membership blocks."""
...
def _encode_context(
    ...
    """Write round, own-turn progress, and dealer indicators."""
...
def encode_policy_observation(information: SearchInformationState) -> Tensor:
    """Encode the actor-visible state as the selected bool[659] layout."""
    ...
    _encode_coffin(observation, information)
    _encode_card_status(observation, information)
    _encode_context(observation, information)
    return observation
...
def select_policy_group(
    ...
    # The validated information state determines both legality and groups, so
    # runtime inference can build the proxy mask without repartitioning it.
    groups = strategic_action_groups(information)
    representative_mask = representative_action_mask_from_groups(groups)
