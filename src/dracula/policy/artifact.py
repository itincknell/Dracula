...
from dracula.policy.model import PolicyModel, PolicyModelError
...
def _model_tensors(model: PolicyModel) -> dict[str, Tensor]:
    """Copy a model's finite float32 tensors into CPU-owned artifact data."""
    ...
    if any(
        ...
        raise PolicyModelError("policy parameters must be finite float32 tensors")
    return tensors


def save_policy_artifact(path: str | Path, model: PolicyModel) -> None:
    """Atomically write one minimal CPU-only policy artifact."""

    if not isinstance(model, PolicyModel):
        raise PolicyModelError("policy artifact requires a PolicyModel")
    destination = Path(path)
    destination.parent.mkdir(parents=True, exist_ok=True)
    temporary = destination.with_name(f".{destination.name}.tmp-{os.getpid()}")
    try:
        # Write and sync a complete temporary artifact before making it visible.
        torch.save(
...
def _load_payload(path: str | Path) -> tuple[bytes, Mapping[str, object]]:
    """Deserialize the minimal artifact envelope from untrusted bytes."""
    ...
    try:
        ...
        raise PolicyModelError("policy artifact could not be loaded") from error
    if (
        not isinstance(payload, dict)
        or set(payload) != {"format", "state_dict"}
        or payload["format"] != POLICY_ARTIFACT_FORMAT
    ):
        raise PolicyModelError("policy artifact format is incompatible")
...
def _validated_tensors(
    ...
    """Validate every tensor before allowing it into the inference model."""

    if not isinstance(raw, dict) or set(raw) != set(expected):
        raise PolicyModelError("policy state-dict keys are incompatible")
    ...
    for name, expected_tensor in expected.items():
        ...
        if (
            ...
            raise PolicyModelError(f"policy tensor is incompatible: {name}")
        tensors[name] = value
    return tensors


def load_policy_artifact(path: str | Path) -> LoadedPolicyArtifact:
    """Load a policy after validating its envelope and every tensor."""

    artifact_bytes, payload = _load_payload(path)
    model = PolicyModel()
    tensors = _validated_tensors(payload["state_dict"], model.state_dict())
    # PyTorch owns tensor installation; all external compatibility checks have
