...
from dracula.game.cards import Card, card_by_id
...
def round_score_deciding_rank(
    queen_line_scores: Sequence[int], king_line_scores: Sequence[int]
) -> int:
    """Return the zero-based ranked line that determines both round scores."""
...
def resolve_round_scores(
    ...
    selected_index = round_score_deciding_rank(queen_line_scores, king_line_scores)
    queen_sorted = sorted(queen_line_scores, reverse=True)
    king_sorted = sorted(king_line_scores, reverse=True)
    return PlayerValues(
        queen=queen_sorted[selected_index],
        king=king_sorted[selected_index],
    )
