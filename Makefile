...
check-opponent:
	@case "$(OPPONENT)" in \
		...
		*) echo "OPPONENT must be search, search-v2, a documented search-v2 student mode, guided, or archive" >&2; exit 1 ;; \
	esac

dev: check-opponent check-api-port
	@set -e; \
	DRACULA_DATABASE_PATH="$(DATABASE)" DRACULA_NARRATION_ENABLED=false DRACULA_OPPONENT_MODE="$(OPPONENT)" DRACULA_SEARCH_SIMULATIONS="$(SEARCH_SIMULATIONS)" DRACULA_SEARCH_EXPLORATION="$(SEARCH_EXPLORATION)" DRACULA_SEARCH_RESPONSE_COMPLETIONS="$(SEARCH_RESPONSE_COMPLETIONS)" DRACULA_RESPONSE_RANKER_ARTIFACT="$(RESPONSE_RANKER_ARTIFACT)" DRACULA_POLICY_VALUE_ARTIFACT="$(GUIDED_ARTIFACT)" DRACULA_GUIDED_SIMULATIONS="$(GUIDED_SIMULATIONS)" DRACULA_POLICY_ARCHIVE="$(CANDIDATE)" DRACULA_POLICY_INFERENCE_PROFILE="$(INFERENCE_PROFILE)" .venv/bin/uvicorn dracula.api.app:app --reload --host 127.0.0.1 --port "$(API_PORT)" & \
	api_pid=$$!; \
