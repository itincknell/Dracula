...
.PHONY: check-api-port check-candidate dev preview dev-api dev-frontend play-report play-game reset-local test-web test-e2e
...
dev: check-candidate check-api-port
	@set -e; \
	DRACULA_DATABASE_PATH="$(DATABASE)" DRACULA_NARRATION_ENABLED=false DRACULA_POLICY_ARCHIVE="$(CANDIDATE)" DRACULA_POLICY_INFERENCE_PROFILE="$(INFERENCE_PROFILE)" .venv/bin/uvicorn dracula.api.app:app --reload --host 127.0.0.1 --port "$(API_PORT)" & \
	api_pid=$$!; \
	trap 'kill $$api_pid 2>/dev/null || true' EXIT INT TERM; \
	DRACULA_API_PROXY_TARGET="http://127.0.0.1:$(API_PORT)" npm --prefix frontend run dev -- --port "$(DEV_PORT)" --strictPort

preview: check-candidate check-api-port
	@set -e; \
	npm --prefix frontend run build; \
	DRACULA_DATABASE_PATH="$(DATABASE)" DRACULA_NARRATION_ENABLED=false DRACULA_POLICY_ARCHIVE="$(CANDIDATE)" DRACULA_POLICY_INFERENCE_PROFILE="$(INFERENCE_PROFILE)" .venv/bin/uvicorn dracula.api.app:app --host 127.0.0.1 --port "$(API_PORT)" & \
	api_pid=$$!; \
	trap 'kill $$api_pid 2>/dev/null || true' EXIT INT TERM; \
	DRACULA_API_PROXY_TARGET="http://127.0.0.1:$(API_PORT)" npm --prefix frontend run preview -- --port "$(PREVIEW_PORT)" --strictPort

dev-api: check-candidate
	DRACULA_DATABASE_PATH="$(DATABASE)" DRACULA_NARRATION_ENABLED=false DRACULA_POLICY_ARCHIVE="$(CANDIDATE)" DRACULA_POLICY_INFERENCE_PROFILE="$(INFERENCE_PROFILE)" .venv/bin/uvicorn dracula.api.app:app --reload --host 127.0.0.1 --port "$(API_PORT)"
