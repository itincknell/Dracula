...
from dracula.api.policy import PolicyTurnRequest, PolicyTurnResult
from dracula.bgc_policy import load_policy_artifact, save_policy_artifact
...
def test_wrong_format_and_tensor_keys_are_rejected(tmp_path: Path) -> None:
    ...
    with pytest.raises(BGCPolicyModelError, match="format is incompatible"):
        load_policy_artifact(wrong_format)

    missing_key = tmp_path / "missing-key.pt"
    state = BGCPolicyModel().state_dict()
    state.pop(next(iter(state)))
    torch.save({"format": "pi1-policy-v1", "state_dict": state}, missing_key)
    with pytest.raises(BGCPolicyModelError, match="keys are incompatible"):
...
def test_incompatible_tensors_are_rejected(
    policy_artifact: tuple[Path, BGCPolicyModel], kind: str
) -> None:
    ...
    with pytest.raises(BGCPolicyModelError, match="tensor is incompatible"):
        load_policy_artifact(modified)


def test_save_load_preserves_logits_and_loads_only_once(
    policy_artifact: tuple[Path, BGCPolicyModel],
